...
if __name__ == '__main__':
    p = PollyWrapper(boto3.client('polly'), boto3.resource('s3'))
    langs = p.get_languages('neural')
    for lang, code in langs.items():
        voices = p.get_voices("neural", code)
        print(lang, code, voices)
    # audio_stream, visemes = p.synthesize(
